...
**Amendment:** 2026-09-23 — [ADR-0025](ADR-0025-argocd-reuses-observability-node.md) and [ADR-0026](ADR-0026-two-flags-one-large-node.md). `enable_observability` and `enable_argocd` (both default false) share this node group. The node exists when either flag is true. Both stacks may run on it together. Terraform does not install the charts.
...
**Decision:** Live Prometheus, Grafana, and Alertmanager run only on a second EKS managed node group. The app group stays 2× t4g.small. The observability group is 1× `t4g.large` (8 GiB), label `workload=observability`, taint `dedicated=observability:NO_SCHEDULE`. Helm values (still git-only until a cluster exists) must set a node selector and toleration for that taint. `node-exporter` stays a DaemonSet on every node. The node is created when `enable_observability` or `enable_argocd` is true (ADR-0026, both default false). A normal workload apply does not create the group or the instance. The next apply with both flags false deletes the node. Neither flag installs charts. Do not apply in this change. Do not invent AWS logging resources. Loki (1Gi, empty disk), FluentBit (DaemonSet on every node), and Zipkin (512Mi, in-memory, namespace `tracing`) share this node for a short session. They use the same node selector and toleration, except FluentBit, which only tolerates the taint so it can run on the app nodes too.
...
| Loki + FluentBit live logs (`PETPLAT-59`) | A later ADR. ADR-0025 gives the node to ArgoCD | Values are in git. Empty disk for the session. 10Gi/50Gi PVs wait on PETPLAT-84. LogQL alert rules are not in this slice. |
...
- Leaving `enable_observability=true` or `enable_argocd=true` on later applies, which keeps the `t4g.large` running.
