...
echo ""
echo "Next: ./scripts/write-backend-config.sh"
echo "      then terraform init -reconfigure -backend-config=terraform/backend.hcl"
echo "      in each environment root (no kms_key_id; do not commit backend.hcl)."
