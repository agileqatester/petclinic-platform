...
In chat, `@aidlc-workflows` / `@saas-ntier-lab` after the workspace is open. `docs/technical-spec.md` is the course contract; **accepted ADR-0001** overrides the old all-public crossword (private nodes + `t4g.micro` NAT instance in the destroyable stack). **Accepted ADR-0013:** EKS lives in `environments/{env}/workload` with NAT, not in network. **Accepted ADR-0014:** ECR lives in `environments/dev/network` (keep), not workload. **Accepted ADR-0015:** RDS + `petclinic/{env}/rds-credentials` live in `environments/dev/workload` (destroy), not network. **Accepted ADR-0016:** Route 53/ACM are optional (gated in network); LBC/ALB wait on EKS apply. **Accepted ADR-0017:** OpenAI SM secret (gated) + ESO IRSA live in `environments/dev/workload` (destroy); ESO install waits on EKS apply. **Accepted ADR-0018:** E-8 writes namespaces and NetworkPolicies only; Helm (E-16) owns Deployments. **Accepted ADR-0019:** E-9 freezes replica, HPA, and PDB numbers for `helm-values/{dev,prod}.yaml`; E-16 writes those files. No Kustomize. No apply. **Accepted ADR-0020:** GitHub OIDC and `petclinic-github-actions-role` live in `environments/dev/network` (keep). This repo authors the tag-update workflow and a reference build workflow; the live build runs in an application fork. No apply. **Accepted ADR-0021:** Prometheus, Grafana, Alertmanager, Loki, FluentBit, and Zipkin values live in `helm-values/observability/` and `helm/zipkin`. Not installed. Not applied. No CloudWatch module. **Accepted ADR-0025:** ArgoCD runs on that same tainted `t4g.large` with memory caps. No `helm_release`. Not applied. **Accepted ADR-0026:** `enable_observability` and `enable_argocd` (both default false) share that node. Either flag creates it. Both true runs both stacks there. Terraform installs neither. **Accepted ADR-0022:** E-13 audits the IAM roles, security groups, and image scans already in git. Security groups match the spec. `Resource: "*"` stays only for `ecr:GetAuthorizationToken` and the upstream load balancer controller policy. No apply. PETPLAT-70 and live NetworkPolicy checks stay open. **Accepted ADR-0023:** ResourceQuota and LimitRange for `petclinic-dev` and `petclinic-prod` only, in `k8s/base/resource-quotas.yaml`. Requests and limits are both CPU 4 and memory 4Gi, pods 30. Published sizes do not admit under `limits.*`. Not applied. **Accepted ADR-0024:** `helm/petclinic-service/` plus `helm-values/{service}.yaml` and `helm-values/{dev,prod}.yaml`. Prod replica, HPA, and PDB are per-service maps. Not installed. Implementers follow the spec as updated by those ADRs.
...
- **Per-env config** in `helm-values/{dev,prod}.yaml` (replicas, HPA, PDB, ECR registry). ResourceQuota and LimitRange are `k8s/base/resource-quotas.yaml` (ADR-0023), not these files.
...
| EKS nodes | 2x t4g.small ARM; optional 1x t4g.large when `enable_observability=true` or `enable_argocd=true` (ADR-0026) | 2x t4g.small ARM |
...
- **Architecture:** CI (GitHub Actions) + CD (ArgoCD). GitHub Actions NEVER deploys directly. **ADR-0020:** OIDC role in the keep network stack. Live `build-push.yml` is copied into an application fork. This repo authors `update-image-tags.yml` and a reference build workflow.
